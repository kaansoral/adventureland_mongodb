...
if(typeof items==="undefined"){ try{ items=require("./items.js").items; }catch(e){ try{ items=require("./design/items.js").items; }catch(e2){} } }
...
var cosmo2_data=[[1,25],[2,25],[3,25],[4,25],[5,22]];
for(var i=0;i<cosmo2_data.length;i++){
	var n=cosmo2_data[i][0], nm=cosmo2_data[i][1];
...
var cosmo3_data=[[1,16],[2,25],[3,24],[4,8]];
for(var i=0;i<cosmo3_data.length;i++){
	var n=cosmo3_data[i][0], nm=cosmo3_data[i][1];
	for(var num=0;num<nm;num++){
		var key=""+n+("0"+num).slice(-2);
...
for(var i=0;i<keys_to_fix.length;i++){
	...
	if(key.indexOf(",")!=-1){
		var t=drops[key];
		delete drops[key];
		var parts=key.split(","); parts.sort();
		key=parts.join(",");
		drops[key]=t;
	}
}
