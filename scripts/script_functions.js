...
function cdate(time) {
	// custom date/time format [12/06/15]
	var time_str = "",
		current = new Date(),
		arr = [current.getHours(), current.getMinutes(), current.getDate(), current.getMonth() + 1, current.getYear()];
	...
	got(url)
		.then((response) => {
			if (response.statusCode == 200) {
				...
				errorf(new Error(`Status code: ${response.statusCode}`));
			}
		})
		.catch((error) => {
			errorf(error);
...
function minify_all(path, files) {
	...
	for (var sub_path in files) {
		for (var i = 0; i < files[sub_path].length; i++) {
			var type = "sqwish",
				file = files[sub_path][i],
				the_path = path + "/" + sub_path + "/"; // previously 'yui-css' but requires Java [16/06/20]
...
function in_args(strs) {
	...
	for (var j = 0; j < strs.length; j++) {
		str = strs[j];
		for (var i = 0; i < process.argv.length; i++) if (process.argv[i] == str) return true;
	}
	return false;
}
