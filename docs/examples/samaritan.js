...
if (typeof SAMARITAN_SETTINGS === "undefined") {
	var SAMARITAN_SETTINGS = {
		...
		stat: {
			...
			maximumAttemptsPerSession: 2,
			rules: [],
			// Example: {name:"helmet", stat:"str", maxItemValue:20000}
		},
	};
}
...
async function samaritanCall(label, operation) {
	try {
		...
		if (samaritanFailure(result) && !["cooldown", "full", "safety", "nothing_to_loot"].includes(result.reason)) samaritanLog(label, label + " failed: " + (result.reason || "unknown"), 60000);
		return result;
	} catch (error) {
...
function on_party_request(name) {
	if (samaritanIsTrusted(name))
		samaritanCall("party request", function () {
...
function on_party_invite(name) {
	if (samaritanIsTrusted(name))
		samaritanCall("party invite", function () {
...
function samaritanPartyTick() {
	...
	var current = get_party() || {};
	if (leader && leader !== character.name && !character.party && !Object.prototype.hasOwnProperty.call(current, character.name))
		return samaritanCall("party request", function () {
	...
	if (leader === character.name && SAMARITAN_SETTINGS.party.inviteMembers) {
		for (var name of SAMARITAN_SETTINGS.party.members || []) {
			if (name !== character.name && !Object.prototype.hasOwnProperty.call(current, name)) {
				samaritanCall("party invite", function () {
					return send_party_invite(name);
				});
				break;
			}
		}
...
function samaritanChooseFarm(force) {
	...
	if (SAMARITAN_SETTINGS.farm.monster !== "auto" && samaritanMonsterAvailable(SAMARITAN_SETTINGS.farm.monster)) {
		...
		var scored = (SAMARITAN_SETTINGS.farm.candidates || [])
			.map(function (type) {
				return { type: type, score: samaritanMonsterScore(type) };
			})
...
async function samaritanSupport() {
	...
	if (type === "priest") {
		var healTarget = samaritanFriendlyTargets()
			.filter(function (entity) {
				return entity.max_hp && entity.hp / entity.max_hp < SAMARITAN_SETTINGS.support.healBelowRatio;
			})
			.sort(function (left, right) {
				return left.hp / left.max_hp - right.hp / right.max_hp;
			})[0];
		if (healTarget && can_heal(healTarget))
			return samaritanCall("heal", function () {
				return heal(healTarget);
			});
		var injuredParty = samaritanFriendlyTargets().filter(function (entity) {
			return entity.max_hp && entity.hp / entity.max_hp < 0.82;
		});
		if (injuredParty.length >= 2 && samaritanSkillReady("partyheal"))
			return samaritanCall("partyheal", function () {
	...
	if (type === "paladin" && character.max_hp && character.hp / character.max_hp < 0.68 && samaritanSkillReady("selfheal"))
		return samaritanCall("selfheal", function () {
			return use_skill("selfheal");
		});
	if (type === "merchant" && SAMARITAN_SETTINGS.support.buffNearbyPlayers) {
		...
		if (luckTarget && samaritanSkillReady("mluck", luckTarget))
			return samaritanCall("mluck", function () {
				return use_skill("mluck", luckTarget);
			});
	}
	if (type === "mage") {
		...
		if (energyTarget && samaritanSkillReady("energize", energyTarget)) {
			...
			if (amount > 0)
				return samaritanCall("energize", function () {
					return use_skill("energize", energyTarget, amount);
				});
		}
	}
	if (type === "rogue" && SAMARITAN_SETTINGS.support.buffNearbyPlayers) {
		...
		if (speedTarget && samaritanSkillReady("rspeed", speedTarget))
			return samaritanCall("rspeed", function () {
				return use_skill("rspeed", speedTarget);
			});
	}
}
...
function samaritanFindTarget(farm) {
	return (
		Object.values(parent.entities || {})
			.filter(function (entity) {
				return samaritanEligibleMonster(entity, farm);
			})
			.sort(function (left, right) {
		...
		Object.values(parent.entities || {})
			.filter(function (entity) {
				return (
					...
					!entity.dead &&
					!entity.rip &&
					!samaritanTargetBlocked(entity) &&
					protectedNames.includes(entity.target) &&
					distance(character, entity) <= protectionRange
				);
			})
			.sort(function (left, right) {
...
async function samaritanCombatSkill(target, farm) {
	...
	if (type === "ranger") {
		...
		if (targets.length >= 3 && character.hp / character.max_hp > 0.8 && samaritanSkillReady("3shot"))
			return samaritanCall("3shot", function () {
				return use_skill("3shot", targets.slice(0, 3));
			});
		if (target.hp > character.attack * 1.4 && samaritanSkillReady("supershot", target))
			return samaritanCall("supershot", function () {
				return use_skill("supershot", target);
			});
	}
	if (type === "rogue") {
		var weaponType = samaritanWeaponType();
		if (weaponType === "dagger" && samaritanSkillReady("quickstab", target))
			return samaritanCall("quickstab", function () {
				return use_skill("quickstab", target);
			});
		if (weaponType === "fist" && samaritanSkillReady("quickpunch", target))
			return samaritanCall("quickpunch", function () {
				return use_skill("quickpunch", target);
			});
	}
...
function samaritanTravelToFarm(farm) {
	...
	smart_move(farm)
		.then(function () {
			SAMARITAN_STATE.travelRetryAt = 0;
		})
		...
		.finally(function () {
			SAMARITAN_STATE.traveling = false;
		});
}
...
function samaritanProgressItemSafe(item, rule, allowStat) {
...
function samaritanCanEquip(item, slot) {
...
function samaritanEquipmentScore(item) {
	if (!item || !G.items[item.name]) return -Infinity;
	var definition = G.items[item.name];
	var level = Math.max(0, Number(item.level) || 0);
	var type = samaritanCharacterType();
	var primary = { warrior: "str", ranger: "dex", rogue: "dex", mage: "int", priest: "int", paladin: "str", merchant: "int" }[type];
	var score = 0;
	for (var stat of ["attack", "armor", "resistance", "hp", "mp", "speed", "range", "str", "dex", "int", "vit", "stat"]) {
		var value = Number(definition[stat]) || 0;
		var growth = definition.upgrade && (Number(definition.upgrade[stat]) || 0);
		var weight = stat === "attack" ? 8 : stat === primary || stat === "stat" ? 6 : ["armor", "resistance"].includes(stat) ? 2 : 1;
		score += (value + growth * level) * weight;
	}
	if (item.stat_type === primary) score += 25;
	return score;
}

function samaritanFindEquipment() {
	var settings = SAMARITAN_SETTINGS.equipment || {};
	if (!settings.enabled || Date.now() - SAMARITAN_STATE.lastEquipmentAt < Math.max(1000, Number(settings.minimumIntervalMs) || 0)) return null;
	for (var rule of settings.rules || []) {
		if (!rule || typeof rule.name !== "string" || typeof rule.slot !== "string") continue;
		var candidates = [];
		for (var i = 0; i < character.items.length; i++) {
			var item = character.items[i];
			if (samaritanProgressItemSafe(item, rule, true) && samaritanCanEquip(item, rule.slot)) candidates.push({ index: i, item: item });
		}
		if (!candidates.length) continue;
		candidates.sort(function (a, b) {
			return samaritanEquipmentScore(b.item) - samaritanEquipmentScore(a.item);
		});
		var equipped = character.slots && character.slots[rule.slot];
		if (equipped && equipped.name === rule.name && samaritanEquipmentScore(equipped) >= samaritanEquipmentScore(candidates[0].item)) continue;
	...
	for (var rule of SAMARITAN_SETTINGS.upgrade.rules || []) {
		...
		if (rule.includeEquipped !== false)
			for (var slot in character.slots || {}) {
				...
				if (samaritanSafeItem(equipped, rule) && (Number(equipped.level) || 0) < rule.maxLevel) return { slot: slot, item: equipped, rule: rule };
			}
	}
	return null;
}

function samaritanFindStat() {
	var settings = SAMARITAN_SETTINGS.stat || {};
	if (!settings.enabled || SAMARITAN_STATE.statAttempts >= settings.maximumAttemptsPerSession) return null;
	for (var rule of settings.rules || []) {
		if (!rule || !["str", "dex", "int", "vit"].includes(rule.stat)) continue;
		for (var i = 0; i < character.items.length; i++) {
			var item = character.items[i];
			if (samaritanProgressItemSafe(item, rule, false) && G.items[item.name] && G.items[item.name].stat) return { index: i, item: item, rule: rule };
		}
		if (rule.includeEquipped !== false)
			for (var slot in character.slots || {}) {
				var equipped = character.slots[slot];
				if (samaritanProgressItemSafe(equipped, rule, false) && G.items[equipped.name] && G.items[equipped.name].stat) return { slot: slot, item: equipped, rule: rule };
			}
	}
	return null;
}
...
async function samaritanEnsureScroll(scrollName, settings) {
	...
	if (character.map !== "main" || distance(character, { x: -465, y: -71 }) > 120) {
		await samaritanCall("scroll travel", function () {
			return smart_move("scrolls");
		});
		return -1;
	}
	await samaritanCall("buy scroll", function () {
...
function samaritanFindSupplyPurchase() {
	...
	for (var rule of settings.items || []) {
		if (
			...
			rule.minimum < 0 ||
			!Number.isSafeInteger(rule.target) ||
			rule.target < rule.minimum ||
			!Number.isSafeInteger(rule.maximumPurchase) ||
			rule.maximumPurchase < 1
		)
			continue;
...
function samaritanFindBankDeposit() {
	if (!SAMARITAN_SETTINGS.bank.enabled || Date.now() < SAMARITAN_STATE.bankRetryAt || (!SAMARITAN_STATE.banking && Date.now() - SAMARITAN_STATE.lastBankAt < SAMARITAN_SETTINGS.bank.minimumIntervalMs))
		return null;
	for (var rule of SAMARITAN_SETTINGS.bank.deposit || []) {
		...
		for (var i = 0; i < character.items.length; i++) {
			...
			if (samaritanSafeItem(item, rule) && total - count >= keep && (rule.maxLevel === undefined || (Number(item.level) || 0) <= rule.maxLevel)) return { index: i, rule: rule };
		}
	}
	return null;
}
...
async function samaritanMaintenance() {
	...
	var statWork = samaritanFindStat();
	var equipmentWork = samaritanFindEquipment();
	...
	if (!supplyWork && !upgradeWork && !compoundWork && !statWork && !equipmentWork && !goldToDeposit && !bankWork) return;
	SAMARITAN_STATE.maintenanceBusy = true;
	try {
		if (smart.moving) stop("smart");
		if (supplyWork) {
			var supplyTravel = await samaritanCall("supply travel", function () {
				return smart_move(supplyWork.name);
			});
			if (samaritanFailure(supplyTravel)) return;
			var supplyPurchase = await samaritanCall("buy supplies", function () {
				return buy_with_gold(supplyWork.name, supplyWork.quantity);
			});
		...
		if (upgradeWork || compoundWork || statWork) {
			...
			if (equippedWork) {
				await samaritanCall("unequip progression item", function () {
					return unequip(equippedWork.slot);
				});
				return;
			}
			if (character.map !== "main" || distance(character, { x: -204, y: -129 }) > 140) {
				await samaritanCall("upgrade travel", function () {
					return smart_move("upgrade");
				});
				return;
			}
			if (upgradeWork) {
				var upgradeIndex = character.items.findIndex(function (item) {
				...
				await samaritanCall("upgrade", function () {
					return upgrade(upgradeIndex, upgradeScroll);
				});
				return;
			}
			if (statWork) {
				...
				var statScroll = await samaritanEnsureScroll(statWork.rule.stat + "scroll", SAMARITAN_SETTINGS.stat);
				if (statScroll < 0) return;
				SAMARITAN_STATE.statAttempts++;
				await samaritanCall("apply stat", function () {
			...
			await samaritanCall("compound", function () {
				return compound(currentIndexes.indexes[0], currentIndexes.indexes[1], currentIndexes.indexes[2], compoundScroll);
			});
			return;
		}
		if (equipmentWork) {
			await samaritanCall("equip upgrade", function () {
				return equip(equipmentWork.index, equipmentWork.slot);
			});
			SAMARITAN_STATE.lastEquipmentAt = Date.now();
			return;
		}
		if (!character.bank) {
			SAMARITAN_STATE.banking = true;
			var bankTravel = await samaritanCall("bank travel", function () {
		...
		if (goldToDeposit > 0) {
			await samaritanCall("bank gold", function () {
				return bank_deposit(goldToDeposit);
			});
			return;
		}
		bankWork = samaritanFindBankDeposit();
		if (!bankWork) return;
		await samaritanCall("bank store", function () {
			return bank_store(bankWork.index, SAMARITAN_SETTINGS.bank.pack);
		});
	} finally {
	...
	try {
		if (character.rip) {
			...
			if (Date.now() - SAMARITAN_STATE.lastRespawnAt > 5000) {
				...
				await samaritanCall("respawn", function () {
					return respawn();
				});
			}
			return;
		...
		if (character.map === "jail" || character.map === "cyberland") {
			...
			if (Date.now() - SAMARITAN_STATE.lastLeaveAt > 10000) {
				...
				await samaritanCall("leave", function () {
					return leave();
				});
			}
			return;
		...
		samaritanPartyTick();
		if (character.max_hp && character.hp / character.max_hp < SAMARITAN_SETTINGS.combat.potionHpRatio)
			await samaritanCall("potion", function () {
				return use_hp_or_mp();
			});
		else if (character.max_mp && character.mp / character.max_mp < SAMARITAN_SETTINGS.combat.potionMpRatio)
			await samaritanCall("potion", function () {
				return use_hp_or_mp();
			});
		var supportResult = await samaritanSupport();
		if (supportResult && !samaritanFailure(supportResult)) {
			await samaritanCall("loot", function () {
				return loot();
			});
			return;
		}
		if (character.max_hp && character.hp / character.max_hp < SAMARITAN_SETTINGS.combat.retreatHpRatio) {
			if (Date.now() - SAMARITAN_STATE.lastRetreatAt > 12000) {
				...
				await samaritanCall("retreat", function () {
					return town();
				});
			}
			return;
		...
		if (immediateThreat && samaritanThreatTooDangerous(immediateThreat)) {
			if (Date.now() - SAMARITAN_STATE.lastRetreatAt > 12000) {
				SAMARITAN_STATE.lastRetreatAt = Date.now();
				await samaritanCall("danger retreat", function () {
					return town();
				});
			}
			return;
		...
		if (!target) {
			if (get_target())
				await samaritanCall("clear target", function () {
					return change_target(null);
				});
		...
		if (get_target() !== target)
			await samaritanCall("target", function () {
				return change_target(target);
			});
		...
		if (can_attack(target)) {
			SAMARITAN_STATE.movingTarget = null;
			SAMARITAN_STATE.moveAttempts = 0;
			await samaritanCall("attack", function () {
				return attack(target);
			});
		} else
			await samaritanCall("combat move", function () {
				return samaritanMoveForCombat(target);
			});
		await samaritanCall("loot", function () {
			return loot();
		});
